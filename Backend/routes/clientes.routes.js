...
router.put('/clientes/:id', async (req, res) => {
  const { id } = req.params;
  const { tipo_documento, ruc, tipo_persona, nombres, apellido_paterno, apellido_materno, nombre_comercial, razon_social, direccion, nombre_contacto, telefono, vendedor, estado, condicion_venta, moneda, correo, direccion_entrega, ag_retencion, ubigeo, cargo_contacto } = req.body;

  try {
    const result = await pool.query(
      ` UPDATE clientes  SET tipo_documento=$1, ruc=$2, tipo_persona=$3, nombres=$4, apellido_paterno=$5, apellido_materno=$6,
      nombre_comercial=$7,  razon_social=$8, direccion=$9, nombre_contacto=$10, telefono=$11, vendedor=$12,
      estado=$13, condicion_venta=$14, moneda=$15, correo=$16,  direccion_entrega=$17,  ag_retencion=$18, ubigeo=$19, cargo_contacto=$20
    WHERE id=$21  RETURNING *  `,
      [
        tipo_documento, ruc, tipo_persona, nombres, apellido_paterno, apellido_materno, nombre_comercial,
        razon_social, direccion, nombre_contacto, telefono, vendedor, estado, condicion_venta,
        moneda, correo, direccion_entrega, ag_retencion, ubigeo, cargo_contacto, id
      ]
    );
